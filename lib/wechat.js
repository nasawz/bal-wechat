 ...
 */
Handler.prototype.middlewarify = function (req, res, next) {
  var that = this;
  ...
  var hack = function (req, res, next) {
 ...
 */
var middleware = function (token,req, res, next, handle) {
  ...
  if (handle instanceof Handler) {
    ...
    return handle.middlewarify(req, res, next);
  } else {
    return new Handler(token, handle).middlewarify(req, res, next);
  }
};
